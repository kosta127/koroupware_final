...
public class UploadController {
	...
	public String uploadForm(MultipartFile file, Model model)throws Exception{
		System.out.println("originalName1:" +file.getOriginalFilename());
		System.out.println("size1:" +file.getSize());
		System.out.println("contentType1:" +file.getContentType());
	...
	//원본파일이름과 파일 데이터를 byte[]로 변환한 정보를 파라미터로 처리해 실제로 파일 업로드
	private String uploadFile(String originalName, byte[] fileData)throws Exception{
		...
		//파일데이터를 파일로 처리 or 복사하는 등의 작업 수행
	...
	public ResponseEntity<String> uploadAjax(MultipartFile file)throws Exception{
		System.out.println("originalName2:" +file.getOriginalFilename());
		System.out.println("size2:" +file.getSize());
		System.out.println("contentType2:" +file.getContentType());
	...
	public ResponseEntity<String> deleteFile(@RequestParam("files[]") String[] files){
		...
		for(String fileName : files){
		    ...
		    if(mType != null){      
		      ...
		      String front = fileName.substring(0,12);//날짜
		      String end = fileName.substring(14);//파일이름
		      
		      System.out.println(front + " : " +end);
		      new File(uploadPath + (front+end).replace('/', File.separatorChar)).delete();//'/'의 위치를 바꾸기 위해 사용
		    }
