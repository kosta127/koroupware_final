...
public class EstimateController {
	...
	public String lateCount(AttendVO attendVO, Model model, EstimateVO estimateVO) throws Exception {
		int dept_no =0;//부서
		...
		if(AttendQuaterList.size()==0){
			...
			for(int x=0; x<AttendQuaterList.size(); x++){
				if((AttendQuaterList.get(x).getEmp_no()!=attendDept_EmpList.get(k).getEmp_no()&&AttendQuaterList.get(x).getResult_year()!=year)){
					resultVO.setQuarterfirst(first_quater/3);
					resultVO.setQuartersecond(secont_quater/3);
					resultVO.setQuarterthird(third_quater/3);
					System.out.println("데이터 넣음");
